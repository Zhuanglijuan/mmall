...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew, User user) {
        //防止横向越权，要检验一下这个用户的旧密码，一定要指定是这个用户,因为我们会查询到一个count(1)，如果不指定Id,那么结果就是true count > 0
        int resultCount = userMapper.checkPassword(MD5Util.MD5EncodeUtf8(passwordOld), user.getId());
        if (resultCount == 0) {
            return ServerResponse.createByErrorMessage("旧密码错误");
        }
        user.setPassword(MD5Util.MD5EncodeUtf8(passwordNew));
        int upsateCount = userMapper.updateByPrimaryKeySelective(user);
        if (upsateCount > 0) {
    ...
    public ServerResponse<User> updateInormation(User user) {
        //username不能被更新
        //email也要校验，校验新的email是不是已经存在，并且存在的email通过相同的话，不能是我们当前的这个用户的。
